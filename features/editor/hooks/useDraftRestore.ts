...
export const useDraftRestore = (options: DraftRestoreOptions = {}) => {
  ...
  const handleManualRestore = useCallback((draft: PhotoDraft) => {
    try {
      ...
      ToastService.show('Taslak geri yüklenemedi'); // Burası Türkçeleştirilecek
    }
  }, [restoreFromDraft]);
...
export const draftUtils = {
  formatDraftAge: (timestamp: number): string => {
    ...
    if (minutes < 1) return 'Şimdi'; // Burası Türkçeleştirilecek
    if (minutes < 60) return `${minutes} dakika önce`; // Burası Türkçeleştirilecek
    if (hours < 24) return `${hours} saat önce`; // Burası Türkçeleştirilecek
    return `${days} gün önce`; // Burası Türkçeleştirilecek
  },
